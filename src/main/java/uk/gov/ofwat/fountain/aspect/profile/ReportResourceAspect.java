...
@Aspect
@Order(301)
public class ReportResourceAspect {
	

	private static Logger log = Logger.getLogger(ReportResourceAspect.class);
	private static Map<String, Integer> callCounts = Collections.synchronizedMap(new HashMap<String, Integer>());
	
	@Around("execution(public * uk.gov.ofwat.fountain.rest.ReportResource.get*(..))")
